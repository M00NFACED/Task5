package ru.vsu.cs.pekshev.T5.tree;
...
public class BinaryTreeAlgorithms {
    ...
    public static <T> int minFind(BinaryTree.TreeNode<Integer> treeNode, Visitor<Integer> visitor) {
        class Inner {
            boolean isItLeft;
            boolean firstLeft = true;
            void minFind(BinaryTree.TreeNode<Integer> node, Visitor<Integer> visitor, int level) {
                if (node == null) {
                    return;
                }
                if (isItLeft){
                    if (firstLeft|| (min > node.getValue())){
                        min=node.getValue();
                        firstLeft = false;
                    }
                }
                visitor.visit(node.getValue(), level);
                isItLeft = true;
                minFind(node.getLeft(), visitor, level + 1);
                isItLeft = false;
                minFind(node.getRight(), visitor, level + 1);
            }

        }
        new Inner().minFind(treeNode, visitor, 0);
        return min;
    }
